...
object App {
  ...
  def renderHighlightedTokens(
    ...
    case class Acc(remainingTokens: Vector[String], curOffset: Int, colorStack: Vector[Rgba], spans: Vector[VdomTag])
    colorLayerings.foldLeft(Acc(tokens, 0, Vector(), Vector())) {
      case (Acc(text, offset, colors, spans), (index, ColorLayering(color, shouldAdd))) =>
        val nextColorSet = if(shouldAdd) colors :+ color else colors.remove(colors.indexOf(color))
        ...
        if(index == offset) {
          ...
          val endingColorStr = if(shouldAdd) colorStr else  {
            NonEmptyList(Rgba.transparent, nextColorSet.toList)
              .reduce((x: Rgba, y: Rgba) => x add y).toColorStyleString
          }
          Acc(
            text.drop(nextIndex), index, nextColorSet,
            spans ++ text.take(nextIndex).zipWithIndex.flatMap {
              case (token, i) => 
                val nextSpaceColorStr = if(i == nextIndex - 1) endingColorStr else colorStr
                if(token == "\n") Vector(<.br(^.key := s"word-${i + offset}"))
                else Vector[Option[VdomTag]](
                  Option(
                  ...
                  Option(
                    <.span(
                      ...
                      ^.style := js.Dynamic.literal("backgroundColor" -> nextSpaceColorStr),
                      " "
                    )
  ...
  def boopickleWebsocket[A: Pickler, B: Pickler] = {
    ...
    new WebSocketConnection2[A, B](
      sendRequest = (socket, req) => Callback {
  ...
  val StringOptField = V.LiveTextField[Option[String]](x => Some(Option(x).filter(_.nonEmpty)), _.getOrElse(""))
  ...
  def userInfoRow(name: String, idOpt: Option[ParticipantId]) = {
    <.div(S.userInfoRow)(
      <.span(S.userInfoMessage)(
        "Your name is ", <.em(name),
        idOpt.map(_.role) match {
          case None => <.span(" and you have no role!")
          case Some(role) => <.span(" and your role is ", <.strong(role.toString)) // TODO stylize role
        }
      )
    )
  }
  ...
  def roleChoiceRow(
    ...
    def facilitatorsDiv = {
      ...
      val roleBoxStyle = if(isCurrent) S.roleBoxCurrent else S.roleBoxNonCurrent
      <.div(roleBoxStyle)(
    ...
    <.div(S.roomRolesRow)(
      <.div(S.roomRolesRow)(
        facilitatorsDiv, observersDiv, downloadButton, disconnectButton
      )
    )
  }
      ...
      <.div(//(S.labeledInputRow)
        ...
        RoundTypeConfig(roundType)(
          "Simultaneous Speeches" -> SumConfigOption(
            ...
            DebateRoundType.simultaneousSpeechesRound) { simulSpeeches =>
            <.span(S.inputRowItem)(
              <.span("Character limit"),
              V.NumberField(simulSpeeches.zoomStateL(DebateRoundType.SimultaneousSpeechesRound.charLimit))
            )
          },
          "Sequential Speeches" -> SumConfigOption(
            DebateRoundType.SequentialSpeechesRound(500),
            DebateRoundType.sequentialSpeechesRound) { seqSpeeches =>
            <.span(S.inputRowItem)(
              <.span("Character limit"),
              V.NumberField(seqSpeeches.zoomStateL(DebateRoundType.SequentialSpeechesRound.charLimit))
            )
          },
          "Judge Feedback" -> SumConfigOption(
            DebateRoundType.JudgeFeedbackRound(true, 500),
            DebateRoundType.judgeFeedbackRound) { judgeFeedback =>
            <.span(S.inputRowItem)(
              V.Checkbox(judgeFeedback.zoomStateL(DebateRoundType.JudgeFeedbackRound.reportBeliefs), "Report probabilities"),
              <.span(
                <.span("Character limit"),
                V.NumberField(judgeFeedback.zoomStateL(DebateRoundType.JudgeFeedbackRound.charLimit))
              )
            )
          }
        )
      ...
      <.form(
        ^.onSubmit ==> (
          (e: ReactEvent) => {
            ...
            sendState(DebateState.debate.composeLens(Debate.setup).set(Some(setup))(debate))
          }
        ),
        <.div(S.labeledInputRow)(
          <.div(S.inputLabel)("Opening Rounds"),
          roundTypeSelect(setupRaw.zoomStateL(DebateSetupRaw.rules.composeLens(DebateRules.fixedOpening)), 0),
        ),
        <.div(S.labeledInputRow)(
        ...
        <.div(S.labeledInputRow)(
          ...
          ScoringFunctionConfig(setupRaw.zoomStateL(DebateSetupRaw.rules.composeLens(DebateRules.scoringFunction)))(
            "Spherical Score" -> SumConfigOption(
              ScoringFunction.SphericalScoreWithLinearPenalty(3, 1),
              ScoringFunction.sphericalScoreWithLinearPenalty) { sphericalScore =>
              <.span(S.inputRowItem)(
                V.LiveTextField.Double(
                  sphericalScore.zoomStateL(ScoringFunction.SphericalScoreWithLinearPenalty.baseCoefficient),
                  Some("Base coefficient")
                ),
                V.LiveTextField.Double(
                  sphericalScore.zoomStateL(ScoringFunction.SphericalScoreWithLinearPenalty.perTurnPenalty),
            ...
            "Quadratic Score" -> SumConfigOption(
              ...
              ScoringFunction.quadraticScoreWithLinearPenalty) { quadraticScore =>
              <.span(S.inputRowItem)(
                V.LiveTextField.Double(
                  quadraticScore.zoomStateL(ScoringFunction.QuadraticScoreWithLinearPenalty.baseCoefficient),
                  Some("Base coefficient")
                ),
                V.LiveTextField.Double(
                  quadraticScore.zoomStateL(ScoringFunction.QuadraticScoreWithLinearPenalty.perTurnPenalty),
                  Some("Per turn penalty")
                )
              )
            },
            "Log Score" -> SumConfigOption(
              ...
              ScoringFunction.logScoreWithLinearPenalty) { logScore =>
              <.span(S.inputRowItem)(
                V.LiveTextField.Double(
        ...
        <.div(S.labeledInputRow)(
          ...
          V.LiveTextArea.String.mod(textarea = TagMod(S.fullWidthInput, ^.rows := 10))(
        ...
        <.div(S.labeledInputRow)(
          ...
          V.LiveTextField.String.mod(input = S.fullWidthInput)(setupRaw.zoomStateL(DebateSetupRaw.question)),
        ),
        <.div(S.labeledInputRow)(
          <.span(S.inputLabel)("Answers"),
          ListConfig.String(setupRaw.zoomStateL(DebateSetupRaw.answers), 1) { (remove, answer, index) =>
            <.div(S.labeledInputRow)(
  ...
  def turnDisplay(
    ...
    turnOpt match {
      ...
      case Some(turn) => turn match {
        case DebateTurnType.SimultaneousSpeechesTurn(remainingDebaters, _) => roleOpt match {
          ...
          case _ => <.span("All debaters are constructing simultaneous speeches.")
        }
        case DebateTurnType.DebaterSpeechTurn(index, _) => roleOpt match {
          case Some(Debater(`index`)) => <.span("It is YOUR TURN to make a speech.")
          case _ => <.span(s"It is Debater ${answerLetter(index)}'s turn.")
        }
        case DebateTurnType.JudgeFeedbackTurn(_, _) => roleOpt match {
          case Some(Judge) => <.span("It is YOUR TURN as judge to give feedback.")
  ...
  def debatePanel(
    ...
    val inProgressSpeechStyle = role match {
      ...
      case Some(Debater(index)) => TagMod(S.answerOutline(index), S.debateWidthOffset(index))
    }

    val currentTurnOpt = debate.currentTurn
    val isUsersTurn = currentTurnOpt.exists {
      case DebateTurnType.SimultaneousSpeechesTurn(_, _) => role.collect { case Debater(_) => () }.nonEmpty
      case DebateTurnType.DebaterSpeechTurn(curSpeaker, _) => role.collect { case Debater(`curSpeaker`) => () }.nonEmpty
      case DebateTurnType.JudgeFeedbackTurn(_, _) => role.collect { case Judge => () }.nonEmpty
    }
    ...
    def makeSimpleVdomFromText(text: String) = {
      <.span(
        text.split("\n").toVector.map(x => Vector(<.span(x): VdomElement)).intercalate(Vector(<.br())).toVdomArray
      )
    }

    def breakNewlines(x: String) = {
      x.split("\n").toVector.map(seg => Vector[VdomNode](<.span(seg))).intercalate(Vector(<.br())).toVdomArray
    }

    def makeSimultaneousSpeechesHtml(speeches: Map[Int, DebateSpeech], speechIndex: Int) = {
      <.div(S.speechRow)(
        ^.key := s"speech-$speechIndex",
        speeches.toVector.sortBy(_._1).toVdomArray { case (debaterIndex, speech) =>
          ...
          <.div(S.speechBox, S.answerBg(debaterIndex))(
            ...
            <.div(S.speechHeader)(
              speech.speaker.name,
              s" ($roleString) ",
              <.span(S.speechTimestamp)(minSecTime(speech.timestamp - setup.startTime)).when(speech.timestamp > 0),
            ),
            speech.content.toVdomArray {
              case SpeechSegment.Text(text) => makeSimpleVdomFromText(text)
              case SpeechSegment.Quote(span) => <.span(
                <.span(S.quoteText)(breakNewlines(ling.Text.renderSpan(setup.sourceMaterial, span))),
    ...
    def makeSpeechHtml(speech: DebateSpeech, style: TagMod, speechIndex: Int) = {
      val roleString = speech.speaker.role.toString
    ...
    def getSpeechLength(speechSegments: Vector[SpeechSegment]) = {
        speechSegments.foldMap {
          ...
          case SpeechSegment.Quote(span) => ling.Text.renderSpan(setup.sourceMaterial, span).size
        }
    }
    ...
    def maybeScrollDebateToBottom = {
      ...
      val isScrolledToBottom = speechesDiv.scrollHeight - speechesJQ.scrollTop() - speechesJQ.outerHeight() < 1
    ...
    val debateSpansWithSpeaker = turns.flatMap {
      case SimultaneousSpeeches(speeches) =>
        speeches.values.toVector.flatMap(speech => speech.allQuotes.map(speech.speaker -> _))
    ...
    LocalSpans.make(Set.empty[ESpan]) { curMessageSpans =>
      val highlights = debateSpansWithSpeaker.map {
        case (id, span) => span -> getSpanColorForRole(id.role)
      ...
      <.div(S.debatePanel)(
        <.div(S.debateSubpanel)(
          SpanSelection2.make(
            ...
            (ispan => curMessageSpans.modState(_ + ispan.toExclusive))
              ...
              val selectingSpanColorOpt = SpanSelection2.Status.selecting.getOption(status).map {
                case SpanSelection2.Selecting(begin, end) => ESpan(begin, end + 1) -> midHighlightColor
              }
              ...
              <.div(S.sourceMaterialSubpanel)(
                renderHighlightedTokens(
                  ...
                  allHighlights,
                  setup.sourceMaterial.indices.toVector.map(
                    i => i -> ((el: VdomTag) => el(^.onMouseMove --> context.hover(i), ^.onClick --> context.touch(i)))
                  ).toMap
                )
              )
          }
        ...
        LocalQuotingMessage.make(
          ...
          def speechInputPanel(submit: Callback, cmdEnterToSubmit: Boolean = true) = {
            <.div(S.speechInputPanel)(
                ...
                textarea = TagMod(
                  ...
                  ^.onKeyDown ==> (
                    (e: ReactKeyboardEvent) => {
                      val submitCB = if(e.keyCode == KeyCode.Enter && (e.metaKey || e.ctrlKey)) {
                        submit
                      } else Callback.empty
                      submitCB
                    }
              ...
              )(currentMessage, didUpdateValue = _ => maybeScrollDebateToBottom),
              <.div(S.speechLengthPanel, S.invalidTextBackground.when(speechIsTooLong))(
                "Length: ",
          ...
          <.div(S.debateSubpanel)(
            <.div(S.speechesSubpanel)(
                ...
                turn match {
                  case SimultaneousSpeeches(speeches) =>
                    if(speeches.size < setup.answers.size) {
                      role.collect { case Debater(index) =>
                        speeches.get(index).map { speech =>
                          val speechStyle = TagMod(S.answerOutline(index), S.pendingBg, S.debateWidthOffset(index))
                          makeSpeechHtml(speech, speechStyle, speechIndex)
                        }
                      }.flatten.toVector
                    } else {
                      Vector(makeSimultaneousSpeechesHtml(speeches, speechIndex))
                    }
                  case DebaterSpeech(speech) =>
                    val speechStyle = speech.speaker.role match {
                      ...
                      case Debater(index) => TagMod(S.answerBg(index), S.debateWidthOffset(index))
                    }
                      ...
                      Option(
                        <.div(
                          ...
                          probabilities.zipWithIndex.toVdomArray { case (prob, index) =>
                            ...
                            <.div(S.answerBg(index), ^.width := pct, ^.color := "white", ^.fontWeight := "bold", ^.flexGrow := "1")(pct)
                          }
                        )
              ...
              userId.whenDefined { userId =>
                makeSpeechHtml(
                  DebateSpeech(userId, -1L, currentMessageSpeechSegments), inProgressSpeechStyle, -1
                ).when(currentMessage.value.size > 0)
              }
            ),
            turnDisplay(role, currentTurnOpt),
            currentTurnOpt.filter(_ => isUsersTurn).whenDefined {
              case DebateTurnType.SimultaneousSpeechesTurn(remainingDebaters: Set[Int], charLimit: Int) =>
                val submit = (
                  if(!isUsersTurn || speechIsTooLong) Callback.empty
                  else userId.foldMap(userId =>
                    CallbackTo(System.currentTimeMillis()).flatMap { time =>
                      val speech = DebateSpeech(userId, time, currentMessageSpeechSegments)
                      val newTurns = role match {
                        case Some(Debater(debaterIndex)) =>
                          turns.lastOption match {
                            ...
                            case Some(SimultaneousSpeeches(speeches)) if speeches.size < setup.answers.size =>
                              turns.updated(turns.size - 1, SimultaneousSpeeches(speeches + (debaterIndex -> speech)))
                            // otherwise, create the record for the turn with our new speech.
                            case _ => turns :+ SimultaneousSpeeches(Map(debaterIndex -> speech))
                          }
                        case _ => turns
                      }
              ...
              case DebateTurnType.DebaterSpeechTurn(debater: Int, charLimit: Int) =>

                val submit = (
                  if(!isUsersTurn || speechIsTooLong) Callback.empty
                  else userId.foldMap(userId =>
                    CallbackTo(System.currentTimeMillis()).flatMap(time =>
                      sendDebate(Debate(Some(setup), turns :+ DebaterSpeech(DebateSpeech(userId, time, currentMessageSpeechSegments))))
                    ) >> currentMessage.setState("")
                  )
                )
              ...
              case DebateTurnType.JudgeFeedbackTurn(reportBeliefs: Boolean, charLimit: Int) =>
                val turnNum = debate.turns.collect { case JudgeFeedback(_, _) => 1 }.sum
                LocalProbs.make(Vector.fill(setup.answers.size)(1.0 / setup.answers.size)) { probs =>
                  val submit = (
                    if(!isUsersTurn || speechIsTooLong) Callback.empty
                    else userId.foldMap(userId =>
                      CallbackTo(System.currentTimeMillis()).flatMap(time =>
                        sendDebate(Debate(Some(setup), turns :+ JudgeFeedback(probs.value, DebateSpeech(userId, time, Vector(SpeechSegment.Text(currentMessage.value))))))
                      ) >> currentMessage.setState("")
                    )
                  )
                    ...
                    <.div(S.row)(
                      ...
                      <.div(S.col, ^.width := s"${barWidthPx * setup.answers.size}px") {
                        val currentScores = setup.answers.indices.map(index =>
                          setup.rules.scoringFunction.eval(turnNum, probs.value, index)
                        )
                        val hypotheticalScores = setup.answers.indices.map(index =>
                          setup.rules.scoringFunction.eval(turnNum + 1, probs.value, index)
                        )

                        val reqdProbs = setup.answers.indices.map { answerIndex =>
                          val scoresNextTurnAfterDeltaIncrease = LazyList.from(0 to 100).map(_.toDouble / 100.0).map { delta =>
                            delta -> setup.rules.scoringFunction.eval(
                              turnNum + 1,
                              ProbabilitySliders.setProb(probs.value, answerIndex, probs.value(answerIndex) + delta),
                              answerIndex)
                          }
                          scoresNextTurnAfterDeltaIncrease.find(_._2 >= currentScores(answerIndex)).map(_._1)
                        }.toVector
                        ...
                        TagMod(
                          <.div(S.col, S.grow)(
                            <.div(S.row)(
                              setup.answers.indices.zip(scores).toVdomArray { case (index, score) =>
                ...
                LocalString.make("") { participantNameLive =>
                  <.div(S.connectDialog)(
                    <.form(
                      ...
                      <.div("Name: ", V.LiveTextField.String(participantNameLive)),
              ...
              DebateRoomLocal.make(DebateState.init) { debate =>
                DebateWebSocket.make(
                  ...
                  case DebateWebSocket.Connected(sendState, _) =>
                    ...
                    val userId = debate.value.participants.find(_.name == userName)
                    val backgroundStyle = S.observerBg

                    <.div(S.roomMainColumn)(
                      roleChoiceRow(roomName, userName, debate.value, sendState, disconnect = connectionSpecOpt.setState(None)),
                      userInfoRow(userName, userId),
                      debate.value.debate.setup match {
                        case None => userId.map(_.role) match {
                          case Some(Facilitator) => facilitatorSetup(debate.value, sendState)
                          case _ => <.div(S.debateColumn)("Waiting for a facilitator to set up the debate.")
                        }
                        case Some(setup) =>
                          def assumeRole(role: Role): Callback = {
                          ...
                          <.div(S.debateColumn, backgroundStyle)(
                            <.div(questionBoxStyle)(
                              ...
                              <.div(S.judgesList)(
                                  "Judges: ",
                                  debate.value.participants.collect { case ParticipantId(name, Judge) => <.span(name) }.toVdomArray,
                              ),
                            ...
                            <.div(S.answerBoxesRow)(
                              setup.answers.zipWithIndex.toVdomArray {
                                case (answer, answerIndex) =>
                                  val isCurrent = userId.map(_.role).collect { case Debater(`answerIndex`) => true }.nonEmpty
                                  val answerBoxStyle = if(isCurrent) S.answerBoxCurrent else S.answerBox
                                  <.div(answerBoxStyle(answerIndex))(
                                    <.div(S.answerTitle)(s"${answerLetter(answerIndex)}. $answer"),
                                    <.div(S.debatersList)(
                                      "Debaters: ",
                                      debate.value.participants.collect {
                                        case ParticipantId(name, Debater(`answerIndex`)) =>
                                          <.span(name)
                                      }.toVdomArray,
                                    ),
                                    ^.onClick --> assumeRole(Debater(answerIndex))
                                  )
                              }
                            ),
                            debatePanel(userId, setup, debate.value.debate, (d: Debate) => sendState(DebateState.debate.set(d)(debate.value)))
                          )
                      }
                    )
                }
