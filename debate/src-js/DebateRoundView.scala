          ...
          case (debaterIndex, speech) =>
            <.div(S.speechBox, S.answerBg(debaterIndex))(
              ...
              speech.content.toVdomArray {
                ...
                case SpeechSegment.Quote(span) => quoteToHTML(source, span)
              }
            )
        }
      )
      ...
      <.div(
        round
          ...
          .whenDefined(roundTime =>
            debateStartTime.whenDefined(startTime =>
              timestampHTML(startTime, roundTime)
            )
          )
          .when(
            roleOpt
        ...
        round match {
          case SimultaneousSpeeches(speeches) =>
            if (speeches.size < numDebaters) {
              ...
              Vector(
                makeSimultaneousSpeechesHtml(source, speeches, debateStartTime, roleOpt)
              ).toVdomArray
            }
          case SequentialSpeeches(speeches) =>
            val speechesToShow = if (speeches.size < numDebaters) {
              if (
                roleOpt.collect { case Facilitator | Debater(_) => () }.nonEmpty
              ) {
