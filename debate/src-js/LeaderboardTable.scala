...
object LeaderboardTable {
  ...
  sealed trait SortableColumn {
    ...
    def getColumnHeader(category: LeaderboardCategory): String =
      this match {
        case Name =>
          "Name"
        case Wins =>
    ...
    def getOrdering: Ordering[RowData] =
      this match {
        case SortableColumn.Name =>
          Ordering.by[RowData, String](_.name)
        case SortableColumn.Wins =>
          Ordering.by[RowData, Int](_.stats.wins.included)
        case SortableColumn.Losses =>
          Ordering.by[RowData, Int](_.stats.wins.excluded)
  ...
  def sortButtons[T, B](sortOrder: StateSnapshot[SortingOrder], sortableColumn: SortableColumn) =
    <.div()(
      <.i(c"bi bi-caret-down-fill")(
        ^.onClick --> sortOrder.setState(SortingOrder(isAscending = false, column = sortableColumn))
      ),
      <.i(c"bi bi-caret-up-fill")(
        ^.onClick --> sortOrder.setState(SortingOrder(isAscending = true, column = sortableColumn))
      )
    )
          ...
          SortableColumn
            ...
            .toVdomArray { column =>
              ...
              <.th(
                ^.key := header,
                header,
                sortButtons(sortableColumn = column, sortOrder = sortOrder)
              )
            }
        )
      ),
      <.tbody(
        sortedRows.toVdomArray { case row =>
          <.tr(
            <.td(row.name),
  ...
  val defaultSortingOrder = SortingOrder(isAscending = true, SortableColumn.Name)
  ...
  def renderSingleLeaderboard(category: LeaderboardCategory, stats: Map[String, DebateStats]) =
    LocalSortingOrder.make(defaultSortingOrder) { sortOrder =>
      <.div(
        ...
        renderRows(category = category, stats = stats, sortOrder = sortOrder)
      )
    }
