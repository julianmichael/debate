...
object LeaderboardTable {
  ...
  def sortButtons[T, B](sortOrderRef: StateSnapshot[SortingOrder], sortableColumn: SortableColumn) =
    <.div()(
      <.i(c"bi bi-caret-down-fill")(
        ^.onClick -->
          sortOrderRef.setState(SortingOrder(isAscending = true, column = sortableColumn))
      ),
      <.i(c"bi bi-caret-up-fill")(
        ^.onClick -->
          sortOrderRef.setState(SortingOrder(isAscending = false, column = sortableColumn))
      )
    )
  ...
  def make() = {
      ...
      val ordering =
        column match {
          case SortableColumn.Name =>
            Ordering.by[RowEntry, String](_.name)
          case SortableColumn.Wins =>
            Ordering.by[RowEntry, Int](_.wins)
          case SortableColumn.Losses =>
            Ordering.by[RowEntry, Int](_.losses)
    ...
    def renderRowEntries(
      ...
      def ourSortButtons(x: SortableColumn) = sortButtons(
        sortableColumn = x,
      ...
      <.table(c"table table-striped")(
        <.thead(
          <.tr(
            <.th("Name", ourSortButtons(SortableColumn.Name)),
            <.th("Wins", ourSortButtons(SortableColumn.Wins)),
            <.th("Losses", ourSortButtons(SortableColumn.Losses)),
            ...
            <.th("Win %", ourSortButtons(SortableColumn.WinPercentage)),
            <.th(customColumnName, ourSortButtons(SortableColumn.CustomRewardValue))
          )
        ),
        <.tbody(
          sortedRows.toVdomArray { case rowEntry =>
            <.tr(
              <.td(rowEntry.name),
              <.td(rowEntry.wins),
              <.td(rowEntry.losses),
              <.td(f"${rowEntry.wins.toDouble / (rowEntry.losses + rowEntry.wins) * 100}%.1f"),
    ...
    def renderLoaded(parsedResponse: ParsedResponse) = {
      ...
      val baseSortingOrder = SortingOrder(isAscending = true, SortableColumn.Name)
      (new LocalState[SortingOrder]).make(baseSortingOrder) { judgeSortOrder =>
        (new LocalState[SortingOrder]).make(baseSortingOrder) { honestSortOrder =>
          (new LocalState[SortingOrder]).make(baseSortingOrder) { dishonestSortOrder =>
            import LeaderboardCategories._
            def f(x: LeaderboardCategory, name: String) = {
              val (sortOrder, rows) =
                x match {
                  case Judge =>
                    (judgeSortOrder, parsedResponse.judge)
                  case HonestDebater =>
                    (honestSortOrder, parsedResponse.honest)
                  case DishonestDebater =>
                    (dishonestSortOrder, parsedResponse.dishonest)
                }
              renderRowEntries(rows = rows, sortOrderRef = sortOrder, customColumnName = name)
            }
            <.div(
              <.h3("Judge"),
    ...
    def render(state: C.State): japgolly.scalajs.react.vdom.VdomElement =
      state match {
        case C.Loading =>
          <.div(<.h3("Loading Leaderboard..."))
        case C.Loaded(x) =>
          renderLoaded(x)
      }

    def leaderboardToRows(x: Map[String, SerializableDebateStats]) = {
      val init: List[RowEntry] = List()
      x.foldLeft(init) { case (acc: List[RowEntry], (k: String, v: SerializableDebateStats)) =>
        acc :+
          RowEntry(name = k, wins = v.wins, losses = v.losses, customRewardValue = v.averageReward)
      }
    }
      ...
      val f =
        for {
          ...
          safeLeaderboardGenerator =
            (key: LeaderboardCategory) => leaderboardToRows(f.data.getOrElse(key, Map()))
