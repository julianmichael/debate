...
object LobbyPage {
  ...
  def make(
    ...
    def enterRoom(isOfficial: Boolean, roomName: String, participantName: String) =
      connectionSpecOpt.setState(Some(ConnectionSpec(isOfficial, roomName, participantName)))
    ...
    LocalString.syncedWithLocalStorage(key = "profile", defaultValue = "") { userName =>
      <.div(S.lobbyContainer, S.spaceyContainer)(
        ...
        LocalLobbyTab.syncedWithSessionStorage(
          ...
          def lobbySelector() =
            List(MyDebates, AllOfficialDebates, PracticeDebates, Leaderboard).toVdomArray(tab =>
              <.li(c"nav-item")(
          ...
          <.div(c"card", ^.textAlign.center)(
            ...
            LocalString.make("") { roomNameLive =>
              ...
              val enter =
                if (canEnter)
                  enterRoom(isOfficial, roomNameLive.value, userName.value)
                else
                  Callback.empty
              ...
              def makeMetadatas(r: RoomStatus) = {
                ...
                val rooms = currentRooms.filter(_.status == r)
                <.div()(
