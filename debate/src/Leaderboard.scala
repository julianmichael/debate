...
object DebateStats {
  ...
  def combineAssignedUsers(debateResult: DebateResult)(
    x: (DebateRole, String)
  ): Chosen[LeaderboardCategories.LeaderboardCategory, Chosen[String, DebateStats]] = {
    val (debaterKey, inner) =
      x match {
        case (debate.Debater(index), name) =>
          val correct =
            if (debateResult.finalJudgement(index) > 0.5) {
              1
            } else {
              0
            }
          ...
          (
            debaterKey,
            Map(name -> DebateStats(Accuracy.Stats(correct, incorrect), Numbers(reward)))
          )
        case (Judge, name) =>
          val correct =
            if (debateResult.finalJudgement(debateResult.correctAnswerIndex) > 0.5) {
              1
            } else {
              0
            }
          ...
          (
            ...
            Map(
              name ->
                DebateStats(Accuracy.Stats(correct, incorrect), Numbers(debateResult.judgeReward))
            )
          )
      }
  ...
  def foldOverDebate(
    ...
    d.result match {
      case None =>
        Chosen(Map.empty)
      case Some(result) =>
        d.setup.roles.toList.foldMap(combineAssignedUsers(result))
    }
...
object LeaderboardCategories {
  ...
  implicit val keyDecoder: KeyDecoder[LeaderboardCategory] = KeyDecoder.instance {
    case "Judge" =>
      Some(Judge)
    case "HonestDebater" =>
      Some(HonestDebater)
    case "DishonestDebater" =>
      Some(DishonestDebater)
    case _ =>
      None
  }
}
...
@JsonCodec
case class SerializableDebateStats(wins: Int, losses: Int, averageReward: Double)

object SerializableDebateStats {
  def ofDebateStats(d: DebateStats): SerializableDebateStats = SerializableDebateStats(
    wins = d.wins.correct,
...
case class Leaderboard(
  data: Map[LeaderboardCategories.LeaderboardCategory, Map[String, SerializableDebateStats]]
)
  ...
  def ofDebateStates(d: List[Debate]) =
    Leaderboard(
      DebateStats
        ...
        .data
        .view
        .mapValues(_.data.view.mapValues(SerializableDebateStats.ofDebateStats).toMap)
        .toMap
    )
