...
/**
  * The full data object maintained on the server for each debate.
  * Sent to the client in full each time there's a change to the debate.
  *
  * @param debate the contents of the debate
  * @param participants the people currently on the page (w/roles)
  */
@Lenses @JsonCodec case class DebateState(
  debate: Debate,
  participants: Set[ParticipantId]
) {
  /** Add a participant. If the participant is already present, potentially change their role. */
...
/**
  * The state of a debate. Persists when people leave; this is the saveable data object.
  *
  * @param setup the debate's rules, structure, question and answer choices.
  * @param turns the sequence of arguments, feedback or other info exchanged in the debate.
  */
...
/**
  ...
  *
  * @param rules the debate's rules/structure
  * @param sourceMaterial the source material visible to the debaters
  * @param question the question to be debated
  * @param answers all answer choices to be debated
  * @param startTime millis from epoch at which the debate was begun
  */
