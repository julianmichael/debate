...
object Serve
  ...
  val jsDepsPathO = Opts
    .option[NIOPath]("jsDeps", metavar = "path", help = "Where to get the JS deps file.")
  ...
  /** Initialize the server state and make the HTTP server
    ...
    *   the full HTTP app
    */
  def makeHttpApp(jsPath: NIOPath, jsDepsPath: NIOPath, saveDir: NIOPath, blocker: Blocker) =
    for {
      ...
      mainChannel <- Topic[IO, Lobby](Lobby(trackedDebaters, officialRooms, practiceRooms))
      pushUpdate = {
        for {
          debaters         <- trackedDebatersRef.get
          officialRoomList <- officialDebates.getRoomList
          practiceRoomList <- practiceDebates.getRoomList
          _ <- mainChannel.publish1(Lobby(debaters, officialRoomList, practiceRoomList))
        } yield ()
      }
  ...
  def service(
    ...
    HttpRoutes.of[IO] {
      ...
      case GET -> Root =>
        Ok(
          debate.Page(jsDepsLocation = jsDepsLocation, jsLocation = jsLocation).render,
