...
case class DebateStateManager(
        ...
        .map { case (roomName, room) =>
          val (debateResult, currentSpeakers) =
            room.debate.debate.map(_.currentTransitions) match {
              case None =>
                (None, None)
              case Some(Left(debateResult)) =>
                (Some(debateResult), None)
              case Some(Right(debateTransitionSet)) =>
                (None, Some(debateTransitionSet.currentSpeakers))
            }
          RoomMetadata(
            name = roomName,
            assignedParticipants = room.debate.debate.map(_.setup.roles).getOrElse(Map()),
  ...
  def toLeaderboard =
    rooms
      .get
      .map { roomMap =>
        Leaderboard.ofDebateStates(roomMap.values.toList.flatMap(_.debate.debate.toList))
      }
