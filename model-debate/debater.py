...
{prompts.NAME_2}: I claim the answer is {self.answers[1]}{separator}\n"""
    ...
    async def run_single_turn(
        self, history, char_limit: int, quote_char_limit: int, turn_type: str
    ):
        ...
        if turn_type == "single debater":
            ...
            rules = prompts.debate_rules(
                word_limit, quote_limit, turn_type == "simultaneous"
            )
        ...
        if turn_type == "single debater":
            self.private = prompts.private_consultation_prompt(
                self.name, word_limit, quote_limit
            )
        else:
            self.private = prompts.private_debate_prompt(
                self.name, word_limit, quote_limit
            )
        self.position = (
            self.private
        ...
        async with aiohttp.ClientSession() as session:
            ...
            ending_prompt = (
                f"Complete the next turn of debate as your role of {self.name}:"
            )
