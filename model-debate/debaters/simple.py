...
class DebaterSimple(DebaterBase):
    separator = "\n---------------------------------------------\n"
 ...
 """
    ...
    def new_turn(self, turn_input: DebaterTurnInput):
        if len(turn_input.turns) == 0:
            instruction = "It's the first turn. Please provide a concise opening argument supported by quotes from the story. Remember to wrap quotes in <quote></quote> tags."
            confirmation = f"{self.name}: "
        else:
            instruction = "Please provide your argument for the next turn. Remember to be concise, use quotes from the story, and attack your opponent's arguments in addition to defending your own. Remember to wrap quotes in <quote></quote> tags."
            confirmation = f"{self.name}: "
        return [
